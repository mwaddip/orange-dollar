...
import { MLDSAPrimitives, D, GAMMA2_1, GAMMA2_2, N, } from "./ml-dsa-primitives.js";
import { abytes, checkHash, cleanBytes, equalBytes, getMessage, getMessagePrehash, randomBytes, splitCoder, validateOpts, validateSigOpts, validateVerOpts, } from "./utils.js";
...
/**
 * ML-DSA instance implementing the full FIPS 204 digital signature scheme.
 *
 * Provides keygen, sign, verify, and prehash operations. Each instance
 * is parameterized for a specific security level (44/65/87).
 */
class MLDSAInstance {
    info = { type: 'ml-dsa' };
    internal;
    ...
    constructor(opts) {
        ...
        const { newPoly, NTT, polyAdd, polySub, polyShiftl, polyChknorm, MultiplyNTTs, RejNTTPoly, W1Vec, ZCoder, publicCoder, secretCoder, sigCoder, } = p;
        ...
        this.internal = {
            ...
            keygen: (seed) => {
                ...
                const s1 = [];
                for (let i = 0; i < L; i++)
                    s1.push(p.RejBoundedPoly(xofPrime.get(i & 0xff, (i >> 8) & 0xff)));
                const s2 = [];
                for (let i = L; i < L + K; i++)
                    s2.push(p.RejBoundedPoly(xofPrime.get(i & 0xff, (i >> 8) & 0xff)));
 ...
 */
export function getDilithium(opts) {
    return new MLDSAInstance(opts);
}
/** ML-DSA-44 for 128-bit security level. Not recommended after 2030, as per ASD. */
export const ml_dsa44 = /* @__PURE__ */ new MLDSAInstance({
    ...PARAMS[2],
    CRH_BYTES: 64,
    TR_BYTES: 64,
    C_TILDE_BYTES: 32,
...
export const ml_dsa65 = /* @__PURE__ */ new MLDSAInstance({
    ...PARAMS[3],
    CRH_BYTES: 64,
    TR_BYTES: 64,
    C_TILDE_BYTES: 48,
...
export const ml_dsa87 = /* @__PURE__ */ new MLDSAInstance({
    ...PARAMS[5],
    CRH_BYTES: 64,
    TR_BYTES: 64,
    C_TILDE_BYTES: 64,
